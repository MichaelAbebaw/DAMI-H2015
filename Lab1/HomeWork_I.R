################################# DAMI Programming Assignment 1: Clustering ##################################
...
d <- sqrt(sum(c(A[1,] - A[2,])^2)) # 5 points
print(d)
...
while(TRUE){
  ...
  for(g in 1:4){ # In this for loop, the position of each centroid is calculated - mean of obervations assigned to each centroid
  ...
  for(i in 1:nrow(x)){ # In this for loop, each observation is reassigned its closest centroid
    for(j in 1:4){ # First to calculate the Euclidean distance between each observation and each centroid
      dis <- sqrt(sum(c(x[i,] - centroid[j,])^2)) # 5 points
...
# 5 points
plot(x, col = km.clusters, pch = 19) # Plot the observation points 
...
for(i in 1:nrow(x)){
  for(j in 1:nrow(x)){
    if(i < j){
      ed <- sqrt(sum(c(x[i,] - x[j,])^2)) # 5 points
      ed1 <- c(ed1, ed)
    }
  }
}
...
hc.single <- hclust(dist(x.part), method = "single")
plot(hc.single)

# Try average with hclust function and plot the dendrogram
hc.average <- hclust(dist(x.part) , method = "average")
plot(hc.average)
